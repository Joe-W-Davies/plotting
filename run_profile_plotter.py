...
def main(options):
    ...
    if options.data.split('.')[-1] == 'root':
        ...
        df_data = pd.read_hdf(options.data, columns=varrs_data+['newULPhoID']) #NOTE NOTE: again adjust for inccorect UL ID name in mc) 
...
if __name__ == "__main__":
    ...
    optionalArgs.add_argument('-B', '--bintype', action='store', type=str)
